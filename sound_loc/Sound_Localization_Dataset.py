...
def audio_loader(sample, neg_sample):
	...
	movie_name = video_path[1]
	video_path = video_path[0]
	movie_name = movie_name[:-4]
	movie_name = "".join(movie_name)
	data_path = os.path.split(video_path)
	data_path = data_path[0]

	audio_file = movie_name + '.wav'
	audio_path = data_path + '/audios/' + audio_file
	fs,signal=wav.read(audio_path)

	sig = signal / 32767.0
	pos_sound_tensor = torch.from_numpy(sig).squeeze().float()
	pos_sound_tensor_size = list(pos_sound_tensor.shape)
	
	neg_video_path = neg_sample.replace('\n','')

	neg_video_path = os.path.split(neg_video_path)
	neg_movie_name = neg_video_path[1]
	neg_video_path = neg_video_path[0]
	neg_movie_name = neg_movie_name[:-4]
	neg_movie_name = "".join(neg_movie_name)
	neg_data_path = os.path.split(neg_video_path)
	neg_data_path = neg_data_path[0]

	neg_audio_file = neg_movie_name + '.wav'
	neg_audio_path = neg_data_path + '/audios/' + neg_audio_file
	neg_fs, neg_signal=wav.read(neg_audio_path)

	neg_sig = neg_signal / 32767.0
	neg_sound_tensor = torch.from_numpy(neg_sig).squeeze().float()
	neg_sound_tensor = neg_sound_tensor[:pos_sound_tensor_size[0]]
...
def image_loader(sample):
	...
	all_frames = glob.glob(os.path.join(data_path+'/images/'+movie_name+'/*.jpg'))
