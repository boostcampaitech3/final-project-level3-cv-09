...
import os, sys
sys.path.append("..")
import models.pytorchi3d.videotransforms as videotransforms
from models.pytorchi3d.pytorch_i3d import InceptionI3d
from models.pytorchi3d.charades_dataset_full import Charades as Dataset
...
class ImageFeatureExtractor:
    ...
    def extract_image_features(self):
        ...
        first_dataset = Dataset(
            self.root, self.mode, self.isTrain, first_transforms, self.save_dir
        )
        first_dataloader = torch.utils.data.DataLoader(
            first_dataset,
        ...
        second_dataset = Dataset(
            self.root, self.mode, self.isTrain, second_transforms, self.save_dir
        )
        second_dataloader = torch.utils.data.DataLoader(
            second_dataset,
            batch_size=self.batch_size,
            shuffle=True,
            num_workers=2,
        ...
        third_dataset = Dataset(
            self.root, self.mode, self.isTrain, third_transforms, self.save_dir
        )
        third_dataloader = torch.utils.data.DataLoader(
            third_dataset,
            batch_size=self.batch_size,
            shuffle=True,
            num_workers=2,
        ...
        fourth_dataset = Dataset(
            self.root, self.mode, self.isTrain, fourth_transforms, self.save_dir
        )
        fourth_dataloader = torch.utils.data.DataLoader(
            fourth_dataset,
            batch_size=self.batch_size,
            shuffle=True,
            num_workers=2,
        ...
        center_dataset = Dataset(
            self.root, self.mode, self.isTrain, center_transforms, self.save_dir
        )
        center_dataloader = torch.utils.data.DataLoader(
            center_dataset,
            batch_size=self.batch_size,
            shuffle=True,
            num_workers=2,
        ...
        for phase in ["0", "1", "2", "3", "4"]:  # 병렬화 하여 속도 개선
            ...
            for data in dataloaders[phase]:
                ...
                if not os.path.exists(
                    f"{os.path.join(self.save_dir, name[0]+'__'+phase)}.npy"
                ):
                    ...
                    np.save(
                        os.path.join(self.save_dir, name[0] + f"__{phase}"),
                        np.concatenate(features, axis=0).reshape(-1, 1024),
                    )
                    saved_list.append(
                        f"{os.path.join(self.save_dir, name[0]+'__'+phase)}.npy"
                    )
